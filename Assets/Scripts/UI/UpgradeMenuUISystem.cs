...
/// <summary>
/// Manager class for tower upgrades. Handles UI and tower object
...
public class UpgradeMenuUISystem : MonoBehaviour, IUISystem {
  ...
  /// <param name="type">String indicating the type of tower upgrade
  /// requested.</param>
  /// </summary>
  ...
  /// <summary>
  /// Sets the <c>Interactable </c> property on buttons for upgrades based on
  /// the provided <c>UpgradeTree</c>. <param name="tree"><c>UpgradeTree </c>
  /// object to set button activation.</param>
  /// </summary>
  ...
  /// <summary>
  /// Creates a new tower where the currently focused tower is and then destroys
  /// the focused tower. <param name="newTowerPrefab">Prefab for the new tower
  /// to create.</param>
  /// </summary>
  void CreateNewTower(GameObject newTowerPrefab) {
    GameObject newTower =
        Instantiate(newTowerPrefab, _focusedTower.transform.position,
                    _focusedTower.transform.rotation);
